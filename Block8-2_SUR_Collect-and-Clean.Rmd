...
* Change `Social media makes me feel connected` to `feel-connected`
* Change `Social media increases my stress` to `feel-increase-stress`
...
In the next session of this series, [Making Sense of Survey Data](Block8-3_SUR_Making-Sense.html), we will be looking at the benefits of making data tidy, or longer.  However, it is worth noting that for certain surveys, keeping the wide structure might be preferable to make it easier for human eyes to scan and creating basic summary tables.  
...
```{r, eval=FALSE}
write_csv(survey_data_widy, "survey_data_wide.csv")
```
...
With just a few steps of data cleaning, we now have 5 data files in our folder:
